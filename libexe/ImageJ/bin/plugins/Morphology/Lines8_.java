...
public class Lines8_ implements PlugInFilter {
	...
	public int setup(String arg, ImagePlus imp) {
		...
		//String none = "None";
		titles[0] = "None";
		titles[wList.length] = "Connectivity";
		...
		gd.addMessage("Lines8_ v2.13 by G. Landini");
		gd.addMessage("Only 'square pixel\' images supported!");

		gd.addCheckbox("White lines on black background",true);
		...
		if (redirected.equals("Connectivity")){
			IJ.run("Duplicate...", "title=Connectivity");
	...
	public void run(ImageProcessor ip) {
		...
		double nz, narea, mcx, mcy, feret, tferet, rads=180/Math.PI, angle=0.0, breadth1, breadth2, minr, maxr, skellen;
			...
			for (x=0; x<xe; x++){
				if (ip.getPixel(x,y)==255){
					...
					if(ip.getPixel(x+1,y)+ip.getPixel(x,y+1)+ip.getPixel(x-1,y+1)+ip.getPixel(x+1,y+1)==0){
						...
						//scan bounding box from top left to bottom right and label {flood) with 254 if pixel is 255 and any neighbour pixel is 254
						...
						if (doIminmax){ //is colour 253
							if (pixs<mi || pixs>ma){ // erase (filter) particles
								...
								if (selectedOption.equals("Coordinates") || selectedOption.equals("CentreOfMass")){ //...but put centre of mass
									ip.setColor(0);
									ff.fill8(x, y);
									ip.setColor(254);
								}
...
void stats(int nc, Vector vg, float [] pars){
	...
	void showAbout() {
		IJ.showMessage("About Lines8_...",
		"Lines8_ by Gabriel Landini,  G.Landini@bham.ac.uk\n"+
		"ImageJ plugin for estimating various statistics of 8-connected lines.\n"+
		"Here, \'PLength\', \'SkelLength\' & \'Area\' are measured based on the polygon given by\n"+
		"the centres of the boundary pixels. \'Area\' disregards \'holes\' in the objects\n"+
		...
		"Note that 1 pixel objects have 0 area, etc.\n"+
		"Filtered objects are deleted from the image");
	}
